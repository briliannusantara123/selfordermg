...
function __construct()
	...
	public function delete($id,$nama,$nomeja,$cek,$sub,$no)
	{
		$nm = str_replace("%20"," ", $nama);
		$this->db->where('id',$id);
    	$this->db->delete('sh_cart');
    	$this->session->set_flashdata('success',$nm.' Deleted');

		if ($cek == 'home') {
    		redirect('cart/home'.'/del');
    	}else{
    		redirect('cart/home'.'/'.$nomeja.'/'.$cek.'/'.$sub.'/'.$no.'/del');
    	}
	}
	public function ubah($id,$nama,$nomeja,$cek,$sub)
	{
    	...
    	$this->session->set_flashdata('success',$nm.' Updated');
    	if ($cek == 'home') {
